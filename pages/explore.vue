...
<template>
    <main>
        <section v-if="categories.data.length === 0">
            <div class="grid gap-4 auto-rows-auto" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr))">
                <PrimarySkeleton v-for="skeleton in skeletons" :key="skeleton" class="h-[180px] w-full" />
            </div>
        </section>
        <section v-else>
            <div class=" grid gap-4 auto-rows-auto" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr))">
                <div v-for="category in categories?.data?.items" :key="category.id"
                    ...
                    class="p-4 relative h-[180px] rounded-md overflow-hidden cursor-pointer ">
