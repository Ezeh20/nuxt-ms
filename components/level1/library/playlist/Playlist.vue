...
<template>
    <main>
        ...
        <section v-else>
            ...
            <section
                :class="`overflow-y-scroll ${isPlayer ? 'h-[72vh]' : 'h-[80vh]'} ${!rightBarActive && 'h-[61.9vh]'}`">
                ...
                <section v-if="activeTab === 2" class="flex flex-col gap-5">
                    <div v-if="artists?.data?.artists?.items?.length > 0" v-for="artist in artists.data.artists.items"
                        :key="artist?.id">
                        <NuxtLink
                            :class="`grid gap-4 text-text-color/90 ${!rightBarActive ? 'text-xs' : 'text-base'} items-center cursor-pointer h-full`">
